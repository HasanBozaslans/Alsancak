...
module.exports.execute = async (client, message, args) => {
    ...
    if (!victim || !time || !ms(time) || reason.length < 1) return message.channel.send(embed.setDescription(`Komutu doğru kullanmalısın! \`Örnek: ${ayar.prefix || ""}vmute @üye [süre (1s/1d/1m/1h) ] [sebep]\``)).then(x => x.delete({timeout: 10000}));
    ...
    victim.roles.add(ayar.voiceMuteRol).catch();
    let cezaID = cdb.get(`cezaid.${message.guild.id}`)+1
    cdb.add(`cezaid.${message.guild.id}`, +1);
    cdb.set("voicemute", { id: victim.id, bitis: Date.now()+ms(time)});
    cdb.set(`punishments.${cezaID}.${message.guild.id}`, { mod: message.author.id, sebep: reason, kisi: victim.id, id: cezaID, zaman: Date.now(), komut: "VOICE-MUTE" });
    cdb.set(`vstatus.${victim.id}.${message.guild.id}`, true);
    cdb.push(`sicil.${victim.id}.${message.guild.id}`, { mod: message.author.id, sebep: reason, id: cezaID, zaman: Date.now(), komut: "VOICE-MUTE" });
    pdb.add(`cezapuan.${victim.id}.${message.guild.id}`, +10);
    pdb.add(`vmuteCez.${message.author.id}.${message.guild.id}`, +1);
    pdb.add(`vmute.${victim.id}.${message.guild.id}`, +1);


    message.channel.send(embed.setDescription(`${victim} adlı üye **${reason}** sebebi ile **${yaziSure}** boyunca ses kanallarında susturuldu. (\`#${cezaID}\`)`));
    if (message.guild.channels.cache.has(ayar.muteLogKanali)) message.guild.channels.cache.get(ayar.muteLogKanali).send(embed.setDescription(`${victim} adlı üye **${reason}** sebebi ile **${yaziSure}** boyunca ${message.author} tarafından ses kanallarında susturuldu. \n\n• Ceza Numarası: \`#${cezaID}\` \n• Ceza Alan: ${victim} (\`${victim.id}\`) \n• Yetkili: ${message.author} (\`${message.author.id}\`) \n• Başlangıç Tarihi: \`${muteAtılma}\` \n• Bitiş Tarihi: \`${muteBitiş}\` \n• Sebep: \`${reason}\``));
...
module.exports.configuration = {
    ...
    description: "Belirtilen üyeyi ses kanallarında susturur."
};
