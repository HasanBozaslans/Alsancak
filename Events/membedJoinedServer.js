...
module.exports = (member) => {
    ...
    if (durum) {
        ...
        if (member.guild.channels.cache.has(ayar.yeniHesapLogKanali)) member.guild.channels.cache.get(ayar.yeniHesapLogKanali).send(`${member} (\`${member.id}\`) adlı üye sunucuya giriş yaptı ancak hesabı yeni açıldığı için cezalıya atıldı. ALSANCAK`);
    }else{
        ...
        if (member.user.username.includes(ayar.yasakliTag)) {
            ...
            member.send(`Yasaklı Tag bulundurduğun için **${member.guild.name}** ALSANCAK sunucuda yasaklı taga atıldın.`).catch();
            return;
        }
