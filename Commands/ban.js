...
module.exports.execute = async (client, message, args) => {
    ...
    if (banLimit >= 5) {
        message.guild.owner.send(embed.setDescription(`${message.author} (\`${message.author.id}\`) Adlı yetkili 30 dakika'da 5 tane ban attığı için ban yetkisi alındı..`)).catch();
    ...
    victim.send(embed.setDescription(`**${message.guild.name}** ALSANCAK Sunucudan **${reason}** Gerekçesiyle Yasaklandın!`)).catch();
    ...
    message.channel.send(embed.setDescription(`${victim} (\`${victim.id}\`) adlı üye **${reason}** sebebi ile ALSANCAK sunucusundan yasaklandı. (\`#${cezaID}\`)`));
